...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "self_maintain",
        "description": (
            "Scans LITE's own source code for bugs and can fix them, OR builds new "
            "features into LITE's own codebase — creating new files or extending "
            "existing ones based on the user's description. For mode=feature, NEVER "
            "ask the user which file or folder to use — leave file_path/file_paths "
            "empty and it will automatically figure out the correct placement (and "
            "whether main.py's tool registration or ui.py's PyQt6 GUI need touching "
            "too) from LITE's own codebase conventions. Every changed/created file is "
            "backed up (if it existed) and automatically rolled back if it doesn't compile, "
            "so this can never leave LITE broken. Changes take effect on the next restart. "
            "Call when the user asks LITE to check itself for bugs, fix a bug in its own "
            "code, clean up its own codebase, OR add/build a new feature/capability into "
            "itself."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "mode":        {"type": "STRING", "description": "scan (report only, no changes) | fix (fix or create one specific file) | feature (build/extend a new capability, possibly across a few files) | auto (scan + fix up to a few files, default)"},
                "file_path":   {"type": "STRING", "description": "Path relative to LITE's own install folder, e.g. 'actions/new_thing.py'. Required for mode=fix. For mode=feature, leave this empty unless the user explicitly names a file — placement is inferred automatically otherwise."},
                "file_paths":  {"type": "ARRAY", "items": {"type": "STRING"}, "description": "For mode=feature when the user explicitly names multiple files. Leave empty (the normal case) to let placement be inferred automatically from the codebase."},
                "description": {"type": "STRING", "description": "For mode=feature: what the new capability should do, in the user's words. For mode=fix: the specific bug, if described — if omitted, LITE detects it itself via compile/lint checks."},
                "issue":       {"type": "STRING", "description": "Alias for description — either works."},
                "max_fixes":   {"type": "INTEGER", "description": "Max number of files to auto-fix in one 'auto' run (default 3) — keeps changes reviewable."},
            },
            "required": []
        }
    },
...
class LiteLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            ...
            elif name == "self_maintain":
                r = await loop.run_in_executor(None, lambda: self_maintain(parameters=args, player=self.ui, speak=self.speak))
                result = r or "Done."
                if r:
                    self.ui.show_content("SELF-REPAIR", r)
