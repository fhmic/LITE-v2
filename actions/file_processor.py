"""
file_processor.py — LITE Universal File Processor
...
def _gemini_client():
    """
    Text-only requests route through the shared AI client (Gemini, falling
    back to Claude or a configured custom/local endpoint if Gemini fails or
    its quota is exhausted). Multimodal requests (image description/OCR,
    etc.) stay on direct Gemini — that fallback chain is text-only.
    """
    ...
    class _W:
        def generate_content(self, contents):
            if isinstance(contents, str):
                ...
                return _ai_generate(contents, model="gemini-3.5-flash")
            _c = genai.Client(api_key=_get_api_key())
            return _c.models.generate_content(model="gemini-3.5-flash", contents=contents)
