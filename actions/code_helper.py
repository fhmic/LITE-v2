...
def _get_gemini(model: str = GEMINI_MODEL):
    """
    Text-generation handle used throughout this file. Routes through the
    shared AI client, which tries Gemini first and automatically falls back
    to Claude or a configured custom/local endpoint if Gemini fails or its
    quota is exhausted — callers don't need to know or care which provider
    actually answered.
    """
    ...
    class _W:
        def generate_content(self, contents):
            return _ai_generate(contents, model=model)
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"lite_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"lite_debug_{int(time.time())}.png"
